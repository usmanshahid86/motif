...
contract MockServiceManager {
    event BitcoinWithdrawalTransactionSigned(
        address indexed pod,
    ...
    function withdrawBitcoinPSBT(
        address pod,
        uint256 amount,
        bytes calldata psbtTransaction
    ) external {
    ...
    function withdrawBitcoinCompleteTx(
        address pod,
        uint256 amount,
        bytes calldata completeTx
    ) external {
...
contract MockAppRegistry {
    ...
    function registerApp(address app) public {
        require(
            ...
            "AppRegistry: app already registered"
        );
    ...
    function deregisterApp(address app) public {
        ...
        require(
            appStatus[app] == AppRegistrationStatus.REGISTERED,
            "AppRegistry: app not registered"
        );
        appStatus[app] = AppRegistrationStatus.UNREGISTERED;
        emit AppRegistrationStatusUpdated(
            app,
...
contract BitcoinPodManagerTest is Test {
    ...
    function _deployProxiedServiceManager()
        internal
    ...
    function testPodCreation() public {
        ...
        vm.prank(operator);
        bitDSMRegistry.registerOperatorWithSignature(
            operatorSignature,
            operator,
            operatorBtcPubKey
        );
        ...
        assertEq(
            pod.getOperator(),
            operator,
            "Pod operator should be set correctly"
        );
        assertEq(
            bytes(pod.getBitcoinAddress()),
            bytes(userBtcAddress),
            "Bitcoin address should be set correctly"
        );
        assertEq(
            pod.getOperatorBtcPubKey(),
            operatorBtcPubKey,
            "Operator BTC public key should be set correctly"
        );
    }
    ...
    function testPodCreationAndOwnership() public {
        ...
        assertTrue(
            podAddress.code.length > 0,
            "Pod address should be a contract"
        );
    }
    ...
    function testDepositFlow() public {
        ...
        vm.prank(user);
        address podAddress = podManager.createPod(
            operator,
            userBtcAddress,
            lockScript
        );
        ...
        BitcoinPodManager.BitcoinDepositRequest memory request = podManager
            .getBitcoinDepositRequest(podAddress);
    ...
    function testWithdrawalPSBTFlow() public {
        ...
        assertEq(
            podManager.getBitcoinWithdrawalAddress(podAddress),
            withdrawAddress
        );
        ...
        vm.prank(address(serviceManager));
        podManager.setSignedBitcoinWithdrawTransactionPod(
            podAddress,
            signedPSBT
        );
    ...
    function testCompleteWithdrawalFlow() public {
        ...
        vm.prank(user);
        podManager.withdrawBitcoinCompleteTxRequest(
            podAddress,
            preSignedTx,
            withdrawAddress
        );
    ...
    function testVerifyBTCAddressGeneric() public {
        ...
        bytes
            memory script = hex"522103cb23542f698ed1e617a623429b585d98fb91e44839949db4126b2a0d5a7320b02103fa33caff610ac48ad20c4bd9fa8d7c5b9a5c56b6d6315343f16cb93e59fafd0252ae";

        // The corresponding bech32 address for the above script (testnet)
        string
            memory expectedBtcAddress = "tb1qccmqzwmm95pkyg9zl09mqm6kv95tulydtrr6rswezhnfdtt5hg9qwd09jn";

        // Mock the operator's BTC public key in the registry
        // First public key from the script above

        bytes
            memory mockOperatorBtcPubKey = hex"03cb23542f698ed1e617a623429b585d98fb91e44839949db4126b2a0d5a7320b0";
        ...
        (bytes memory pubKey1, bytes memory pubKey2) = BitcoinUtils
            .extractPublicKeys(script);
        console.log("pubKey1 length", pubKey1.length);
        console.log(
            "mockOperatorBtcPubKey length",
            mockOperatorBtcPubKey.length
        );
        assertEq(
            pubKey1.length,
            mockOperatorBtcPubKey.length,
            "Invalid public key Lenght"
        );
        ...
        bool isBtcAddress = btcAddressTestContract.verifyBTCAddress(
            expectedBtcAddress,
            script,
            mockOperatorBtcPubKey
        );
    ...
    function testFailVerifyBTCAddressInvalidOperator() public {
        bytes
            memory script = hex"522102c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee521021c1db6e604a4909a6e70f1994e37df6dfdcb19c8ee4c9648f37087e5f36388b352ae";
        string
            memory btcAddress = "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7";

        // Call with non-operator address should fail
        vm.prank(address(0xbad));
        BTCAddressTestContract btcAddressTestContract = new BTCAddressTestContract();
        btcAddressTestContract.verifyBTCAddress(
            btcAddress,
            script,
            operatorBtcPubKey
        );
    ...
    function testWithdrawalRequestCancellation() public {
        ...
        // Initiate withdrawal
        string
            memory withdrawAddress = "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx";
        vm.prank(user);
        podManager.withdrawBitcoinPSBTRequest(podAddress, withdrawAddress);
       ...
       assertTrue(
           BitcoinPod(podAddress).getPodState() ==
               IBitcoinPod.PodState.Inactive
       );
        ...
        assertEq(podManager.getBitcoinWithdrawalAddress(podAddress), "");
        assertTrue(
           BitcoinPod(podAddress).getPodState() == IBitcoinPod.PodState.Active
       );
    }
    ...
    function testCannotCancelWithdrawalAfterPSBTSubmission() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IBitcoinPodManager.WithdrawalTransactionAlreadySubmitted.selector,
                podAddress
            )
        );
        podManager.cancelWithdrawalRequest(podAddress);
    ...
    function testCannotCancelNonExistentWithdrawal() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IBitcoinPodManager.NoWithdrawalRequestToCancel.selector,
                podAddress
            )
        );
        podManager.cancelWithdrawalRequest(podAddress);
...
contract BTCAddressTestContract is BitcoinPodManager {
    function verifyBTCAddress(
        string calldata btcAddress,
        bytes calldata script,
        bytes memory operatorBtcKey
